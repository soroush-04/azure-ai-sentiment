...
def text_to_speech(text):
    """Convert text to speech and stream it directly."""
    ...
    if speech_synthesis_result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
        # If synthesis is complete, get audio data as bytes
        audio_data = speech_synthesis_result.audio_data
        audio_stream = io.BytesIO(audio_data)

        # Send as HTTP response with audio content type
        response = HttpResponse(audio_stream, content_type='audio/wav')
        response['Content-Disposition'] = 'attachment; filename="outputaudio.wav"'

        return response
    else:
        return HttpResponse("Speech synthesis failed.", status=400)
